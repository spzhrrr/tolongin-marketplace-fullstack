  ...
  try {
    ...
    document.getElementById("list").innerHTML = j.length
      ...
      <div class="card card-pad">
        ...
        <div class="flex gap-sm mt-2"><a class="btn btn-secondary btn-sm" href="#/jobs/${x.id}" data-testid="view-job-${x.id}">Lihat (${x.applicationsCount || x.applicationCount || 0} pelamar)</a></div>
      </div>`,
          )
          .join("")}</div>`
      : empty("Belum ada job");
      ...
      b.addEventListener("click", async () => {
        try {
          const r = await api.post(
            "/services/" + b.dataset.toggle + "/toggle-active",
          );
